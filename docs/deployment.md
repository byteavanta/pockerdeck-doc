# Docker Deployment

This page covers deploying PockerDeck to a production server using Docker.

---

## Minimal `docker-compose.yml`

The simplest production configuration:

```yaml title="docker-compose.yml"
services:
  web:
    image: ceco556/pockerdeck:latest
    ports:
      - "8000:8000"
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "python", "-c", "import urllib.request; urllib.request.urlopen('http://localhost:8000/')"]
      interval: 30s
      timeout: 5s
      retries: 3
      start_period: 10s
```

Deploy with:

```bash
docker compose up -d
```

---

## Exposing PockerDeck on a custom domain

### 1. Point your DNS

Create an **A record** for your domain (e.g. `poker.example.com`) pointing to your server's public IP.

### 2. Set up a reverse proxy with TLS

=== "Caddy (recommended)"

    Install Caddy, then create `/etc/caddy/Caddyfile`:

    ```caddyfile
    poker.example.com {
        reverse_proxy localhost:8000
    }
    ```

    Caddy automatically obtains and renews a Let's Encrypt TLS certificate.

    ```bash
    systemctl reload caddy
    ```

=== "Nginx + Certbot"

    Install Nginx and Certbot, then:

    ```bash
    certbot --nginx -d poker.example.com
    ```

    Add a location block for WebSocket proxying (`/etc/nginx/sites-available/pockerdeck`):

    ```nginx
    server {
        listen 443 ssl;
        server_name poker.example.com;

        # TLS managed by Certbot
        include /etc/letsencrypt/options-ssl-nginx.conf;
        ssl_certificate     /etc/letsencrypt/live/poker.example.com/fullchain.pem;
        ssl_certificate_key /etc/letsencrypt/live/poker.example.com/privkey.pem;

        location / {
            proxy_pass         http://localhost:8000;
            proxy_http_version 1.1;
            proxy_set_header   Upgrade $http_upgrade;
            proxy_set_header   Connection "upgrade";
            proxy_set_header   Host $host;
            proxy_set_header   X-Real-IP $remote_addr;
        }
    }
    ```

    ```bash
    nginx -t && systemctl reload nginx
    ```

!!! warning "WebSocket headers are mandatory"
    If `Upgrade` and `Connection` headers are not forwarded, participants will fail to connect via WebSocket and will see repeated "Disconnected" errors.

---

## Updating to a newer image

```bash
docker compose pull
docker compose up -d
```

---

## Persisting rooms across restarts

!!! warning "In-memory storage"
    PockerDeck currently stores all room data **in memory**. Rooms are lost when the container restarts. This is intentional for simplicity — planning poker sessions are ephemeral by nature.

    A future release may add optional persistence. For now, teams should simply create a new room after a restart.

---

## Resource requirements

PockerDeck is extremely lightweight:

| Resource | Typical usage |
|----------|--------------|
| RAM | ~50 MB |
| CPU | Negligible (idle) |
| Disk | ~200 MB (image) |

It runs comfortably on the smallest cloud VMs (1 vCPU / 512 MB RAM).
